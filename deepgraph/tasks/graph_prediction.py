...
from ..data.dataset import (
    BatchedDataDataset,
    BatchedDataDataset_Substructure,
    BatchedDataDataset_Substructure_Cache,
    TargetDataset,
    GraphormerDataset,
    EpochShuffleDataset,
)
...
@dataclass
class GraphPredictionSubstructureConfig(GraphPredictionConfig):
    add_substructure: str = field(default='transform') # pre, transform

    use_lmdb_cache: bool= field(default=True)

    extra_method: str= field(default='adj')

    local_attention_on_substructures: bool=field(default=True)

    debug_model:bool = field(default=False)

    valid_on_test: bool = field(default=False)

    multiprocessing: bool = field(default=False)

    num_processes: int = field(default=64)

    dataset: str= field(default='zinc')

    split: str= field(default='given')

    root_folder: str= field(default='./datasets')



    id_type: str = field(default='cycle_graph')

    induced: bool = field(default=True)

    edge_automorphism: str = field(default='induced')

    ks: str =field (default='[8]')

    id_scope: str =field (default='global')

    custom_edge_list: str= field(default='none',metadata={"help": "custom_edge_list"})

    directed:  bool = field(default=False)

    directed_orbits: bool = field(default=False)

    subsampling:bool = field(default=True)

    sampling_mode: str=field(default='min_set_cover_random') # random  shortest_path  min_set_cover

    not_only_unused_nodes:bool=field(default=False)

    sampling_redundancy:int=field(default=2)

    sampling_stride:int=field(default=5)

    sampling_random_rate: float=field(default=0.1)

    sampling_random_init:bool = field(default=True)

    must_select_sub:str=field(default='cycle_graph')


    lmdb_dir:str=field(default='cache')

    lmdb_root_dir:str=field(default='./')

    recache_lmdb:bool = field(default=False)




    use_transform_cache: bool=field(default=False)

    transform_cache_number:int = field(default=100)

    recache_transform: bool=field(default=False)

    transform_dir:str=field(default='transform')

@register_task("graph_prediction_substructure", dataclass=GraphPredictionSubstructureConfig)
class GraphPredictionSubstructureTask(FairseqTask):
    """
    Graph prediction (classification or regression) task.
    """
    ...
    def load_dataset(self, split, combine=False, **kwargs):
        ...
        if self.cfg['recache_transform']:
            batched_data = BatchedDataDataset_Substructure_Cache(
                batched_data,
                max_node=self.max_nodes(),
                multi_hop_max_dist=self.cfg.multi_hop_max_dist,
                spatial_pos_max=self.cfg.spatial_pos_max,
            ...
            batched_data = BatchedDataDataset_Substructure(
                batched_data,
                max_node=self.max_nodes(),
                multi_hop_max_dist=self.cfg.multi_hop_max_dist,
                spatial_pos_max=self.cfg.spatial_pos_max,
...
@dataclass
class GraphPredictionSubstructureWithFlagConfig(GraphPredictionSubstructureConfig):
    flag_m: int = field(
        default=3,
        metadata={
            "help": "number of iterations to optimize the perturbations with flag objectives"
...
@register_task("graph_prediction_substructure_with_flag", dataclass=GraphPredictionSubstructureWithFlagConfig)
class GraphPredictionSubstructureWithFlagTask(GraphPredictionSubstructureTask):
    """
    Graph prediction (classification or regression) task.
    """
